...
import {
  readEventVisibilityRulesFromDb,
} from "../src/lib/events/event-visibility-rules-store";

async function main() {
  const { db, sql } = connectDatabase(true);
  try {
    const rules = await readEventVisibilityRulesFromDb(db);
    if (!rules) {
      throw new Error("event_visibility_rules 尚未写入数据库。");
    }
    const rows = await db
      .select({
        id: events.id,
    ...
    for (const row of rows) {
      const sanitizedTitle = sanitizeEventernoteEventTitle(
        row.title,
